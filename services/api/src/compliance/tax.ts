...
export interface TaxRates {
    ...
    pst: number;
    hst: number;
    qst: number; // Quebec specific
}

const PROVINCIAL_RATES: Record<ProvinceCode, TaxRates> = {
...
export class TaxService {
    calculateTax(amountCents: number, province: ProvinceCode, isZeroRated: boolean = false) {
        if (isZeroRated) {
