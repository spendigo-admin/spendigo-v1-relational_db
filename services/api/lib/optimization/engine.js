...
exports.SmartCartEngine = void 0;
...
class SmartCartEngine {
    ...
    async optimize(request, candidates) {
        // 1. Filter candidates by distance (max 10km for example)
        const validStores = candidates.filter(store => calculateDistance(request.userLocation.lat, request.userLocation.lng, store.location.lat, store.location.lng) <= 10);
        ...
        result.explanation = (0, explainability_1.generateExplanation)(result);
        return result;
    }
}
exports.SmartCartEngine = SmartCartEngine;
