...
const MerchantDeals: React.FC = () => {
    const { getStore, updateStoreDeals } = useMarketplace();
    const { can, user } = useAuth();
    const storeId = user?.storeId || '1';
    const store = getStore(storeId);
    const availableProducts = useMemo(() => store?.products || [], [store?.products]);
    const hasWriteAccess = can('deals:write');
    ...
    const handleCreateDeal = () => {
        ...
        const newDeal: Deal = {
            ...
            value: valueNum,
            bogoType: dealConfig.type === 'bogo' ? dealConfig.bogoType : undefined,
        ...
        // Integrate with Global Marketplace Context
        if (status === 'active') {
    ...
    const deleteDeal = (id: string) => {
        ...
        if (confirm('Are you sure you want to end this deal?')) {
            ...
            // Sync Removal with Global Store
                            ...
                            <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                                ...
                                <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                                    ...
                                    <div className="flex items-center gap-2 border-l border-[var(--glass-border)] pl-4">
                                        ...
                                        {hasWriteAccess ? (
                                            <button onClick={() => deleteDeal(deal.id)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="End Deal">
                                                🗑️
                                            </button>
                                        ) : (
