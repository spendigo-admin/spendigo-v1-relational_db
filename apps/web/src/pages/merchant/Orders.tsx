...
const MerchantOrders: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden bg-[var(--surface-1)]">
            ...
            <div className="px-6 pb-4 md:hidden overflow-x-auto flex gap-2 no-scrollbar">
                {[
                    { id: 'placed', label: '🔔 New' },
                    { id: 'preparing', label: '👨‍🍳 Prep' },
                    { id: 'on_hold', label: '⏳ Hold' },
                    { id: 'out_for_delivery', label: '🛍️ Ready / Route' },
                    { id: 'delivered', label: '✅ Done' }
                ].map(status => (
                    <button
                        key={status.id}
                        onClick={() => setMobileStatusFilter(status.id)}
                        className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${mobileStatusFilter === status.id
                            ? 'bg-[var(--brand-primary)] text-white shadow-lg shadow-[var(--brand-primary)]/30'
                            : 'bg-white border border-[var(--glass-border)] text-[var(--text-muted)]'
                            }`}
                    >
                        {status.label}
                        {/* Badge count */}
                        <span className={`ml-2 px-1.5 py-0.5 rounded-full text-[10px] ${mobileStatusFilter === status.id ? 'bg-white/20' : 'bg-gray-100'}`}>
                    ...
                    <div className="flex-1 min-h-0 p-6 pt-0 overflow-hidden">
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 h-full">
                            ...
                            {[
                                ...
                                { id: 'out_for_delivery', label: '🛍️ Ready for Pickup / 🛵 On Route', color: 'border-purple-500' },
                                { id: 'delivered', label: '✅ Delivered / Done', color: 'border-green-500' }
                            ].map(col => (
                                <div
                                    key={col.id}
                                    className={`flex flex-col h-full bg-[var(--surface-1)]/50 rounded-xl border border-[var(--glass-border)] overflow-hidden transition-all duration-300
                                ${/* Mobile Logic: Show only if matches filter. Desktop: Always show all */ ''}
                                ${mobileStatusFilter === col.id ? 'flex' : 'hidden md:flex'}
                                `}
                                >
                                    ...
                                    <div>
                                        ...
                                        <div className="text-sm text-[var(--text-muted)] flex items-center gap-1">
                                            ...
                                            {selectedOrder.customerPhone ? (
                                                <a href={`tel:${selectedOrder.customerPhone}`} className="hover:text-[var(--brand-primary)] hover:underline">
                                                    {selectedOrder.customerPhone}
                                                </a>
                                            ) : (
                                                <span className="italic opacity-50">No phone provided</span>
                                            )}
                                        </div>
                                    </div>
