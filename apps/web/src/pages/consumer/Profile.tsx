    ...
    return (
        <div className="bg-[var(--surface-0)] min-h-screen animate-fade-in pb-20">
            ...
            <div className="max-w-3xl mx-auto p-4 min-h-[60vh]">
                {/* ACCOUNT TAB */}
                {activeTab === 'account' && (
                    <div className="glass-panel-premium rounded-[2.5rem] p-6 md:p-10 animate-fade-in border border-gray-200 bg-white/50">
                ...
                {activeTab === 'addresses' && (
                    <div className="space-y-6 animate-fade-in pb-10">
                        ...
                        {profile.addresses.map(addr => (
                            <div key={addr.id} className={`glass-panel-premium rounded-[2rem] p-8 transition-all duration-500 border border-gray-200 bg-white/70 hover:bg-white hover:shadow-xl group relative overflow-hidden ${addr.isDefault ? 'ring-2 ring-[var(--brand-primary)]' : ''}`}>
                ...
                {activeTab === 'orders' && (
                    <div className="space-y-8 animate-fade-in min-w-0 overflow-hidden pb-10">
                        <div className="flex items-center justify-between px-2 mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] bg-white/50 px-3 py-1 rounded-full border border-gray-200">{orders.length} Total</span>
                        </div>
                        {orders.length === 0 ? (
                            ...
                            orders.map(order => (
                                <Link key={order.id} to={`/order/${order.id}`} className="block relative glass-panel-premium rounded-[2.5rem] p-8 transition-all duration-500 border border-gray-200 bg-white/70 hover:bg-white hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] hover:translate-y-[-4px] group overflow-hidden">
                                    ...
                                    <div className="flex justify-end gap-3">
                                        <button
                                            ...
                                            className="px-6 py-2.5 bg-white text-[var(--text-muted)] text-[10px] font-black uppercase tracking-widest rounded-xl border border-gray-200 hover:bg-gray-50 disabled:opacity-50 transition-all flex items-center gap-2"
                                        >
                    ...
                    <div className="space-y-10 animate-fade-in pb-10">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-3 mb-8 p-2">
                            <input 
                                ...
                                className="w-full sm:flex-1 px-5 py-4 bg-white/50 border border-gray-200 rounded-[1.25rem] outline-none focus:border-[var(--brand-primary)] focus:ring-4 focus:ring-[var(--brand-primary)]/10 text-sm font-bold placeholder:text-[var(--text-muted)]/50 transition-all shadow-sm"
                            />
                            <button 
                        ...
                        {wishlistItems.length > 0 && (
                            <div className="grid grid-cols-1 gap-6">
                                {wishlistItems.map((item, i) => (
                                    <div key={i} className="glass-panel-premium rounded-[2rem] p-6 flex items-center gap-6 group border border-gray-200 bg-white/70 hover:bg-white hover:shadow-xl hover:translate-x-2 transition-all duration-300 relative overflow-hidden">
                                        <div className="w-20 h-20 rounded-2xl bg-white flex-shrink-0 overflow-hidden border border-gray-200 shadow-sm group-hover:scale-105 transition-transform duration-500">
                                            {item.image ? (
