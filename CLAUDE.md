...
- `AuthContext` — Firebase Auth + Firestore user profiles, RBAC permissions via `can(permission)`. User roles: `consumer | merchant | admin`. Merchant sub-roles: `OWNER | MANAGER | STAFF | MARKETING`. **`subscriptionTier` source**: for merchant users the value is read from the store doc (`stores/{storeId}.subscriptionTier`), not the user doc — ensures the tier reflects the store's actual billing state rather than a potentially stale user field.
...
- [apps/web/src/hooks/useStoreActivePrices.ts](apps/web/src/hooks/useStoreActivePrices.ts) — Builds a live cheapest-price map for a store by subscribing to both `deals` and `flyers` subcollections. Exposes `getMinPrice(productId, candidatePrice)` — used when adding to cart to guarantee the lowest currently-active price is applied (prevents stale UI price vs. server validation mismatches).
- [apps/web/src/hooks/useDealQuality.ts](apps/web/src/hooks/useDealQuality.ts) — Historical deal analytics: badge generation (`historic_low`, `great_deal`, etc.), percentile ranking, and predicted next-sale date. Reads from the `flyer_analytics` Firestore collection.
- [apps/web/src/hooks/useCookieConsent.ts](apps/web/src/hooks/useCookieConsent.ts) — Manages cookie consent state (`accepted` / `declined` / `null`) persisted to localStorage.
...
- `triggers/` — Firestore-triggered functions:
  ...
  - `revertExpiredDeals` (`revertExpiredDeals.ts`) — pubsub scheduled every 30 min (Toronto timezone); queries `merchant_products` where `discount_valid_until` is in the past and reverts `price` back to `original_price`, clearing `sale_price`, `on_sale`, `discount_label`, and `discount_valid_until`. Prevents checkout failures caused by stale deal prices surviving past expiry (server-side `placeOrder` validates against current `merchant_products.price`).
...
Tests live in `tests/unit/` and use **Vitest**. Run from repo root with `npm test`. Tests currently cover the **SmartCart module** (10 test files): optimizer, comparison engine, price matrix, unit price normalizer, trip consolidation, single-store simulator, price normalization, and simulation.
