...
- **Glassmorphic Surfaces**: Backdrop filters (`blur-md`) and layered opacity (`bg-white/80`) are tuned to ensure text contrast ratios meet WCAG 2.1 AA (4.5:1) regardless of background content movement.
...
- **Sticky Header**: The glassmorphic navigation bar includes focus-trapping and `z-index` management to remain accessible and visible during high-speed scroll events.
...
- **Store Identification Stickers**: Floating product labels include descriptive `aria-label` attributes (e.g., "Product sold by [Store Name]") to ensure source recognition for screen reader users.
...
- **TalkBack** (Android)
- **Lighthouse Accessibility Audit** (Goal: 100/100 score)
