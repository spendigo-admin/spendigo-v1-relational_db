...
- **Trigger**: New order creation in `/orders` via `onOrderCreated` trigger.
- **Design**: Rich HTML table generated by `sendOrderEmails.ts` with inline CSS for cross-client compatibility.
- **Audit**: Every confirmation is linked to a `txnId` in the Forensic Audit ledger.

### 3.2 Status Progress alerts
- **Trigger**: Merchant updates order from `placed` -> `preparing` -> `out_for_delivery`. Handled by `sendOrderStatusUpdate` function.
...
- **Action**: Approval emails are written directly to the `mail` collection from the `StoreManagement.tsx` admin module.
